...
class SignalSelfCrossAttention(nn.Module):
    def __init__(self, n_heads: int, d_model: int, d_k: int, d_v: int, dropout: float = 0.1, SoftMaxClass=cvnn.MagMinMaxNorm,
                 conv=cvnn.default_cvconv1d, kernel_size=3, b=1, gamma=2):
        super(SignalSelfCrossAttention, self).__init__()
        
        self.AA = cvnn.CVMultiheadAttention(n_heads, d_model, d_k, d_v, dropout, SoftMaxClass)
        self.AB = cvnn.CVMultiheadAttention(n_heads, d_model, d_k, d_v, dropout, SoftMaxClass)
        self.BA = cvnn.CVMultiheadAttention(n_heads, d_model, d_k, d_v, dropout, SoftMaxClass)
        self.BB = cvnn.CVMultiheadAttention(n_heads, d_model, d_k, d_v, dropout, SoftMaxClass)
...
class DualRadarFusion(nn.Module):
    ...
    def __init__(self, conv, in_channels, kernel_size, n_heads, d_model, 
                 d_v, d_k, dropout, SoftMaxClass, b, gamma):
        super(DualRadarFusion, self).__init__()
        
        self.d_model = d_model
        
        self.head1 = conv(in_channels=in_channels, out_channels=d_model, kernel_size=kernel_size)
        self.head2 = conv(in_channels=in_channels, out_channels=d_model, kernel_size=kernel_size)
        
        self.R_SSCA = SignalSelfCrossAttention(n_heads=n_heads, d_model=d_model, d_k=d_k, d_v=d_v, dropout=dropout, SoftMaxClass=SoftMaxClass,
                                               conv=conv, kernel_size=kernel_size, b=b, gamma=gamma)
        self.k_SSCA = SignalSelfCrossAttention(n_heads=n_heads, d_model=d_model, d_k=d_k, d_v=d_v, dropout=dropout, SoftMaxClass=SoftMaxClass,
                                               conv=conv, kernel_size=kernel_size, b=b, gamma=gamma)
...
class SSCANet_Small(nn.Module):
    ...
    def __init__(self, args: argparse.Namespace, conv=cvnn.default_cvconv1d, SoftMaxClass=cvnn.MagMinMaxNorm):
        ...
        self.dual_radar_fusion = DualRadarFusion(conv=conv, in_channels=in_channels, kernel_size=kernel_size, n_heads=n_heads, d_model=d_model, 
                                                 d_v=d_v, d_k=d_k, dropout=dropout, SoftMaxClass=SoftMaxClass, b=b, gamma=gamma)
...
class SSCANet_Big(nn.Module):
    ...
    def __init__(self, args: argparse.Namespace, conv=cvnn.default_cvconv1d, SoftMaxClass=cvnn.MagMinMaxNorm):
        ...
        self.domain_fusion = DomainFusion(conv=conv, in_channels=2*d_model, kernel_size=kernel_size, n_heads=n_heads, d_model=d_model, 
                                                 d_v=d_v, d_k=d_k, dropout=dropout, SoftMaxClass=SoftMaxClass, b=b, gamma=gamma)
        
        self.refinement = RefinementBlock(conv=conv, in_channels=4*d_model, out_channels=out_channels, kernel_size=kernel_size, act=act, n_res_blocks=B)
