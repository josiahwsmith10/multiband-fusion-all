...
def test_complex_gradient():
    x = torch.ones(1, 1, 5, dtype=torch.complex64) + 1j * torch.ones(
        1, 1, 5, dtype=torch.complex64
    )
    ...
    # Set conv weight
    c.conv.weight = torch.nn.Parameter(
        torch.ones_like(c.conv.weight) + 1j * torch.ones_like(c.conv.weight)
    )
    ...
    for i in range(5):
        ...
        print(
            "nc weight:",
            nc.conv_r.weight.squeeze().detach().numpy()
            + 1j * nc.conv_i.weight.squeeze().detach().numpy(),
        )
...
def multi_head_attention():
    ...
    m = cvnn.CVMultiheadAttention(
        n_head=8, d_model=128, d_k=64, d_v=64, dropout=0.0
    ).to(args.device)
...
def test_SSCA():
    ...
    m = SignalSelfCrossAttention(
        n_head=8, d_model=32, d_k=64, d_v=64, dropout=0.0, kernel_size=3
    ).to(args.device)
