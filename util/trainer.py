...
class Trainer:
    ...
    def train(self):
        ...
        self.logs["log loss"], self.logs["train losses"] = self.loss.end_log(
        ...
        with torch.no_grad():
            ...
            self.logs["val_log loss"], self.logs["val losses"] = self.loss.end_log(
                len(self.loader_val)
            )
    ...
    def print_loss(self):
        ...
        print(
            f"training \t min:{self.logs['log loss min']:.3f}, max:{self.logs['log loss max']:.3f}, cur:{self.logs['log loss']:.3f}"
        )
        print(
            f"validation \t min:{self.logs['val_log loss min']:.3f}, max:{self.logs['val_log loss max']:.3f}, cur:{self.logs['val_log loss']:.3f}\n"
        )
    ...
    def save_checkpoint(self):
        ...
        SaveModel(
            ...
            loss=self.loss,
            trainer=self,
            PATH=self.save_path + f"epoch{epoch}_{self.args.model_name}.pt",
        )
